...
function ProductDetailList() {
  ...
  const hideProduct = async (product_detail_id, is_hidden) => {
    ...
    if (window.confirm(`Bạn có chắc chắn muốn ${newStatus} sản phẩm này?`)) {
      try {
        const response = await axios.patch(`http://localhost:8000/product/${newStatus}/${product_detail_id}`, {}, {
              ...
              <tr key={detailItem.product_detail_id}>
                ...
                <td>
                  {detailItem.productImage ? (
                    <img src={`http://localhost:8000/img/${detailItem.productImage.img_url}`} alt="Sản phẩm" style={{ width: '80px', height: '80px' }} />
                  ) : (
                    "Không có hình ảnh"
                  )}
                </td>
